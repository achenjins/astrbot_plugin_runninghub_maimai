...
_RECORD_FIELDS = (
    ...
    "region",
    "user_id",
    "group_id", "platform_id", "platform_name",
    "created_at",
    "updated_at",
    "message",
    ...
    "inputs", "inputs_ready", "stage", "started_at", "finished_at", "hidden", "actions",
)
...
class TaskJournal:
        ...
        def _read() -> list[dict[str, Any]]:
            ...
            for item in raw:
                ...
                if record.get("delivery_status") == "sending":
                    record.update(delivery_status="uncertain", message="进程在发送期间退出，请确认是否收到后使用 /wf补发")
    ...
    def recoverable_records(self) -> list[dict[str, Any]]:
        return [r for r in self.records() if r["status"] in ACTIVE_STATUSES
                or (r["status"] == STATUS_SUCCESS and r.get("delivery_status") == "pending")]
    ...
    def _normalize_record(self, raw: dict[str, Any]) -> dict[str, Any]:
        ...
        record["actions"] = copy.deepcopy(raw.get("actions")) if isinstance(raw.get("actions"), dict) else {}
        record["inputs"] = copy.deepcopy(raw.get("inputs")) if isinstance(raw.get("inputs"), dict) else {}
        for name in ("started_at", "finished_at"):
            record[name] = raw.get(name) or 0
        record["inputs_ready"] = bool(raw.get("inputs_ready"))
        record["hidden"] = bool(raw.get("hidden"))
        ...
        async with self._lock:
            ...
            for index, record in enumerate(self._records):
                if str(record.get("task_id") or "") == task_id:
                    ...
                    if merged["status"] in {STATUS_SUCCESS, STATUS_FAILED, STATUS_CANCELLED} and not merged.get("finished_at"):
            ...
            if record["status"] in {STATUS_SUCCESS, STATUS_FAILED, STATUS_CANCELLED} and not record["finished_at"]:
