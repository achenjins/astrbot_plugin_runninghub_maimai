...
| --- | --- |
| `/wf运行 <工作流名> [描述]` | 运行工作流 |
| `/wf 保存提示词` | 从最近 5 条扩写提示词中选择并持久保存 |
| `/wf 提示词重跑` | 复用最近一次扩写提示词，在原工作流重新上传文件/修改参数后运行 |
| `/wf 提示词` | 列出已保存提示词；回复数字运行，回复 `删除1` 删除第 1 条 |
| `/wf 上传提示词` | 上传一条可在 `/wf 提示词` 中选择运行的提示词 |
| `/wf 上传提示词模板` | 上传 `.md` / `.txt` 扩写提示词模板 |
| `/wf中断` | 中断输入会话或按编号取消任务 |
| `/wf工作流` | 列出已配置工作流 |
...
提示词缓存按平台和用户隔离。每次带描述的工作流成功提交后，插件会保留最近 5 条实际使用的提示词；列表显示扩写前描述。重跑和选择已保存提示词时直接复用扩写结果，不会再次调用 LLM，文件与可编辑参数仍按当前工作流重新收集。

### 提示词命令用法

1. 输入 `/wf 保存提示词`，回复数字，再发送保存描述。
2. 输入 `/wf 提示词`：回复 `1` 运行第 1 条，回复 `删除1` 删除第 1 条，回复 `取消` 退出。
3. 输入 `/wf 提示词重跑`，直接重跑最近一条提示词。
4. 输入 `/wf 上传提示词`，选择对应工作流后发送 `.md` 或 `.txt` 文件；文件名会作为提示词描述，并加入 `/wf 提示词` 列表。
5. 输入 `/wf 上传提示词模板`，发送 `.md` 或 `.txt` 文件，保存到持久化的 `prompt/` 模板目录，可在工作流配置页面选择。两种上传都会自动识别 UTF-8、UTF-16、GB18030/GBK、Big5 等常见编码并统一转成 UTF-8。

提示词记录保存在 AstrBot 数据目录的 `data/plugin_data/runninghub_workflow_adapter/prompt_library.json`，不会写入插件源码目录。

NapCat 发送远程视频时，如果视频消息段被 NapCat 拒绝，插件会自动改用文件消息发送同一视频链接。
